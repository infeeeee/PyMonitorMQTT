...
class RamSensor(Entity):
    ...
    def Update(self):
        ...
        self.SetTopicValue(TOPIC_MEMORY_PERCENTAGE,
                           psutil.virtual_memory().percent, self.ValueFormatter.TYPE_PERCENTAGE)
        self.SetTopicValue(TOPIC_SWAP_PERCENTAGE,
                           psutil.swap_memory().percent, self.ValueFormatter.TYPE_PERCENTAGE)

        if self.GetOption(self.consts.ADVANCED_INFO_OPTION_KEY):
            # Virtual memory
            self.SetTopicValue(TOPIC_MEMORY_TOTAL,
                               psutil.virtual_memory().total, self.ValueFormatter.TYPE_BYTE)
            self.SetTopicValue(TOPIC_MEMORY_AVAILABLE,
                               psutil.virtual_memory().available, self.ValueFormatter.TYPE_BYTE)
            self.SetTopicValue(TOPIC_MEMORY_USED,
                               psutil.virtual_memory().used, self.ValueFormatter.TYPE_BYTE)
            self.SetTopicValue(TOPIC_MEMORY_FREE,
                               psutil.virtual_memory().free, self.ValueFormatter.TYPE_BYTE)
            # Swap memory
            self.SetTopicValue(
                TOPIC_SWAP_TOTAL, psutil.swap_memory().total, self.ValueFormatter.TYPE_BYTE)
            self.SetTopicValue(
                TOPIC_SWAP_USED,  psutil.swap_memory().used, self.ValueFormatter.TYPE_BYTE)
            self.SetTopicValue(
                TOPIC_SWAP_FREE, psutil.swap_memory().free, self.ValueFormatter.TYPE_BYTE)
